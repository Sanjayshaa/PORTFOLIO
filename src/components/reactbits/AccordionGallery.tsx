...
const AccordionGallery: React.FC<AccordionGalleryProps> = ({
      ...
      panels.forEach((panel, i) => {
        ...
        if (showLabels && bar && text) {
          if (isActive) {
            ...
            if (skills) tl.to(skills, { opacity: 1, y: 0, duration: dur, ease }, 0.1);
          } else {
            tl.to([bar, text], { opacity: 0, x: -14, duration: dur * 0.6, ease }, 0);
            if (skills) tl.to(skills, { opacity: 0, y: 10, duration: dur * 0.6, ease }, 0);
          }
        }
      });
  ...
  return (
    <div
      ...
      role="list"
      aria-label="Technology stack accordion gallery"
    >
      {items.map((item, i) => {
        const isActive = i === active;
        return (
            ...
            {showLabels && (
              <span className="ag-panel__label" aria-hidden="true">
                ...
                {item.skills && item.skills.length > 0 && (
                  <span className="ag-panel__skills" ref={el => { skillsRefs.current[i] = el; }}>
                    {item.skills.map((skill, sIdx) => (
                      <span key={sIdx} className="ag-panel__skill-tag">
                        {skill}
                      </span>
                    ))}
                  </span>
